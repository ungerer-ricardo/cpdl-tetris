...
Tradutor::setConexao(Conexao *_conexao)
{
    ...
    QObject::connect(this->conexao, SIGNAL(connected()),this, SLOT(connectSuccessSlot()));
...
Tradutor::incomingMessage(QString _mensagem)
{
    ...
    switch (traduzido.comando)
    {
        ...
        case ENCA:
            qDebug() << "Parseadobutton ENUM:" << traduzido.comando;
            qDebug() << "Parseado ID:" << traduzido.idJogador;
...
void
Tradutor::connectSuccessSlot()
{
    qDebug() << "to aqui";
    emit this->connectSuccessSignal();
}
